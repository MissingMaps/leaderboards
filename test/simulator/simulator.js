...
function Simulation (hashtags) {
  ...
  this.centers = R.times(mapathon(hashtags), 10);
  this.m = moment();
}
...
function mapathon (hashtags) {
  return () => {
    var country = chance.pick(countries);
    return {
      lat: country.geometry.coordinates[1],
      lon: country.geometry.coordinates[0], hashtag: chance.pick(hashtags)};
  };
}
